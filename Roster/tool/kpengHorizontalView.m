...
- (CGFloat)calculateRowWidth:(NSString *)string {
    NSDictionary *dic = @{NSFontAttributeName:[UIFont systemFontOfSize:16]};  //指定字号
...
//    }
    ...
    CGFloat BF =0;
    float buttonW =  (SCREEN_WIDTH-60) / 4.0;
    for (int index =0; index <_localListArr.count; index++) {
        ...
        _btn.titleLabel.font =_nomalFont?_nomalFont:[UIFont systemFontOfSize:16];
...
//        } else {
         ...
         _btn.frame = CGRectMake(index*buttonW, 0, buttonW, 40);
//            _btn.frame =CGRectMake(BF-[self calculateRowWidth:_localListArr[index]]+index*15, 0, frame.size.width/4, frame.size.height-1);
        
       // }
       
        [_btn setTitleColor: [UIColor colorWithRed:164 / 255.0 green:165 / 255.0 blue:169 / 255.0 alpha:1] forState:UIControlStateNormal];
     ...
     self.contentSize =  CGSizeMake(_localListArr.count*buttonW, 0);
    _line =[UIView new];
...
- (void)btnHitAction:(UIButton*)sender {
    [_localBtnListArr enumerateObjectsUsingBlock:^(UIButton * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [obj setTitleColor:[UIColor colorWithRed:164 / 255.0 green:165 / 255.0 blue:169 / 255.0 alpha:1]  forState:UIControlStateNormal];
        obj.titleLabel.font =_nomalFont?_nomalFont:[UIFont systemFontOfSize:16];
    }];
    [sender setTitleColor:_selectedColor?_selectedColor:[UIColor colorWithRed:100 / 255.0 green:100 / 255.0 blue:100 / 255.0 alpha:1] forState:UIControlStateNormal];
    sender.titleLabel.font =_selectedFont?_selectedFont:[UIFont boldSystemFontOfSize:18];
    ...
    if (_itemClickCallBack) {
        _itemClickCallBack(sender.tag,sender.titleLabel.text,sender.frame.origin.x);
    }
...
-(void)setLocalIndex:(NSInteger)localIndex {
    ...
    [_localBtnListArr enumerateObjectsUsingBlock:^(UIButton * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (idx ==localIndex) {
            [obj setTitleColor:_selectedColor?_selectedColor:[UIColor colorWithRed:100 / 255.0 green:100 / 255.0 blue:100 / 255.0 alpha:1] forState:UIControlStateNormal];
            obj.titleLabel.font =_selectedFont?_selectedFont:[UIFont boldSystemFontOfSize:18];
        } else {
            [obj setTitleColor:[UIColor colorWithRed:164 / 255.0 green:165 / 255.0 blue:169 / 255.0 alpha:1] forState:UIControlStateNormal];
