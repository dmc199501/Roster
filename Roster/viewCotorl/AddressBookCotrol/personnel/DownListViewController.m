...
- (void)viewDidLoad
{
    ...
    self.tv.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
//    if (IS_IPHONE_5) {
...
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id<UIViewControllerTransitionCoordinator>)coordinator {
    ...
    if (size.width > size.height) { // 横屏
        // 横屏布局 balabala
        self.tv.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_WIDTH);
        [self.tv reloadData];
          previewController.view.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_WIDTH);
