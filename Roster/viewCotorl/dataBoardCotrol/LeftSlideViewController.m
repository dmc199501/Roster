...
- (void)openLeftView;
{
    ...
    // NSLog(@"------%@",NSStringFromCGPoint(kMainPageCenter));
    
       self.mainVC.view.center = kMainPageCenter;
   
    //self.mainVC.view.backgroundColor = [UIColor yellowColor];
...
- (void)disableTapButton
{
    for (UIButton *tempButton in [_mainVC.view subviews])
    {
        ...
        [tempButton setUserInteractionEnabled:NO];  NSLog(@"5555");
    }
    //单击
...
//    if (touch.view.frame.origin.y<88) {
//        
//         return NO;
//    }
    if(touch.view.tag == 0)
