...
@interface MCChangePswViewController ()
...
@implementation MCChangePswViewController
...
- (void)viewDidLoad {
    ...
    _IDField  =[[UITextField alloc] initWithFrame:CGRectMake(20, 40, SCREEN_WIDTH-40, 20)];
    ...
    _IDField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"输入旧密码" attributes:@{NSForegroundColorAttributeName: color2}];
    ...
    pwdButtonTwo = [[UIButton alloc]initWithFrame:CGRectMake(SCREEN_WIDTH-37, BOTTOM_Y(_IDField)+5, 17, 12)];
    ...
    _passwordTwo.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"确认新密码" attributes:@{NSForegroundColorAttributeName: color2}];
    _passwordTwo.font = [UIFont systemFontOfSize:16];
    [_passwordTwo becomeFirstResponder];
    _passwordTwo.textColor = [UIColor blackColor];
    ...
    [pwdButtonthree setBackgroundImage:[UIImage imageNamed:@"可视"] forState:UIControlStateNormal];
...
- (void)next{
    ...
    {
        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:nil message:@"请输入8位以上密码" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alertView show];
        return;
    }
    ...
    {
        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:nil message:@"请输入8位以上新密码" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alertView show];
        return;
    }
    ...
    {
        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:nil message:@"请重复密码" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alertView show];
        return;
    }
    ...
    [sendDict setObject:UUID forKey:@"uuid"];
    
    [MCHttpManager PutWithIPString:BASEURL_ROSTER urlMethod:@"/adminuser/ModfiyPwd" parameters:sendDict success:^(id responseObject) {
        ...
        if ([[NSString stringWithFormat:@"%@",dicDictionary[@"code"]] isEqualToString:@"0"] )
        {
            ...
            [self leave];
             [STTextHudTool showSuccessText:@"密码修改成功" withSecond:2];
            ...
            [STTextHudTool showSuccessText:dicDictionary[@"message"]];
        ...
        [STTextHudTool showSuccessText:@"修改失败!"];
